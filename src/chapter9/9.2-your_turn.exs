defmodule PragProg do
  ...
  defp parse_line(line), do: (
    ...
    Order.new id: id, ship_to: binary_to_atom(ship_to), net_amount: String.rstrip(net_amount)
  )
  ...
  def format_orders(orders), do: (
    ...
    IO.puts "|  ID | Ship to | Net amt | Sales tax |"
    IO.puts "|-------------------------------------|"
    Enum.each orders, fn order -> format_order(order) end
    IO.puts "|_____________________________________|"
  )

  # TODO: This annoys me; I don't know how to do rounding and number formatting in Elixir.

  defp format_order(order=Order[id: id, ship_to: ship_to, net_amount: net_amount, sales_tax: sales_tax]), do: (
    IO.puts "| #{id} |    #{ship_to}   |  #{net_amount} | #{sales_tax} |"
  )
end
...
PragProg.format_orders(processed_orders)
